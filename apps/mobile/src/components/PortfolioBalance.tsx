...
const PortfolioBalance = (props: PortfolioBalanceProps) => {
  ...
  const trendIcon = (valueChange > 0) ? 
    <FontAwesomeIcon icon={faArrowTrendUp} style={{ color: gainColor }} size={20} /> :
    <FontAwesomeIcon icon={faArrowTrendDown} style={{ color: lossColor }} size={20} />
  return (
    <Box alignItems="center" my="4">
      {
        !isHidden && <Box alignItems="center">
          ...
          <HStack space={3}>
            <Text fontSize="xl">{changeValueIndicator}${valueChange.toString().replace(/^\-/, "")}</Text>
            <Text fontSize="xl" color={changePercentColor}>{changePercent}%</Text>
            <Box mt="1">
              { trendIcon }
            </Box>
          </HStack>
