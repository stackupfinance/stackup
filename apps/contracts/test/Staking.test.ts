...
describe('Staking', () => {
  ...
  describe('receive', () => {
    ...
    it('accepts ETH from anyone', async () => {
      ...
      expect(await ethers.provider.getBalance(staking.address)).to.be.equal(amount)
    })
  })
  ...
  describe('stake', () => {
    ...
    const itStakesCorrectly = () => {
      context('when the given delay is lower than the specified by the contract', () => {
        const delay = UNLOCK_DELAY - 1

        it('reverts', async () => {
          await expect(staking.stake(delay, amount, { from: paymaster })).to.be.revertedWith('Staking: Low unstake delay')
        })
      })

      context('when the given delay is higher than or equal to the specified by the contract', () => {
  ...
  describe('unstake', () => {
    context('when the paymaster did not deposit', async () => {
      ...
      it('reverts', async () => {
        await expect(staking.unstake({ from: paymaster })).to.be.revertedWith('Staking: Deposit not staked yet')
      })
    })
    ...
    context('when the paymaster has already deposit', async () => {
      ...
      context('when the paymaster has already staked', async () => {
        ...
        context('when the paymaster is unstaking', async () => {
          ...
          it('reverts', async () => {
            await expect(staking.unstake({ from: paymaster })).to.be.revertedWith('Staking: Unstaking in progress')
          })
        })
      })
    })
  ...
  describe('withdraw', () => {
    ...
    context('when the paymaster did not deposit', async () => {
      ...
      it('reverts', async () => {
        await expect(staking.withdraw(paymaster, { from: paymaster })).to.be.revertedWith('Staking: Withdraw amount zero')
      })
    })
    ...
    context('when the paymaster has already deposit', async () => {
      ...
      context('when the paymaster has already staked', async () => {
        ...
        context('when the paymaster did not unstaked yet', async () => {
          ...
          it('reverts', async () => {
            await expect(staking.withdraw(paymaster, { from: paymaster })).to.be.revertedWith('Staking: Cannot withdraw')
          })
        })
