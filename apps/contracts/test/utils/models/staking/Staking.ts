...
export default class Staking {
  ...
  async getDeposit(paymaster: Account): Promise<{ amount: BigNumber, unstakeDelaySec: BigNumber, withdrawTime: BigNumber }> {
  ...
  async deposit(to: Account, amount: BigNumber, params: TxParams = {}): Promise<ContractTransaction> {
    return this.with(params).depositTo(toAddress(to), { value: amount.toString() })
  }

  async stake(delay: BigNumberish, amount: BigNumberish, params: TxParams = {}): Promise<ContractTransaction> {
  ...
  async withdraw(recipient: Account, params: TxParams = {}): Promise<ContractTransaction> {
