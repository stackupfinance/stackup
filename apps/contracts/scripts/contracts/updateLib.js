...
async function main() {
  return Promise.all([
    fs.writeFile(
      path.resolve(__dirname, "../../lib/contracts/source/EntryPoint.json"),
      prettier.format(JSON.stringify(EntryPoint), { parser: "json" })
    ),
    fs.writeFile(
      path.resolve(__dirname, "../../lib/contracts/source/WalletProxy.json"),
      prettier.format(JSON.stringify(WalletProxy), { parser: "json" })
    ),
    fs.writeFile(
      path.resolve(__dirname, "../../lib/contracts/source/Wallet.json"),
