...
const log = (msg) => logger.info(`JOB ${types.recoverAccount}: ${msg}`);

const recoverAccount = (queue) => {
  queue.define(types.recoverAccount, async (job) => {
    ...
    if (!transactionHash) {
      ...
      queue.now(types.recoverAccount, { transactionHash: tx.hash, ...job.attrs.data });
      return;
    }
    ...
    if (txStatus === status.pending) {
      ...
      queue.now(types.recoverAccount, { ...job.attrs.data });
    } else {
      ...
      pusherService.pushRecoverAccountUpdates(channelId, { status: txStatus, transactionHash });
